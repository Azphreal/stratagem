...
fn test_move_basic() {
    ...
    let expected = vec![Move::new(e0, Coord::from("f0").unwrap()),
...
fn test_move_into_terrain() {
    ...
    let expected = vec![Move::new(e1, Coord::from("f1").unwrap()),
...
fn test_move_into_enemy() {
    ...
    let e0 = Coord::from("e0").unwrap();
    board.set_tile(e0, Tile::Piece(Piece::Marshall, Colour::Red));
    board.set_tile(Coord::from("e1").unwrap(), Tile::Piece(Piece::Flag, Colour::Blue));
...
fn test_move_scout() {
    ...
    board.set_tile(Coord::from("h0").unwrap(), Tile::Piece(Piece::Flag, Colour::Red));
    // Enemy three spaces below; should capture.
    board.set_tile(Coord::from("b0").unwrap(), Tile::Piece(Piece::Flag, Colour::Blue));
